...
points_scored <- function(gameData){
...
pointsScored <- matrix(rep(0, 35*35), nrow = 35, ncol=35)
rownames(pointsScored) <- 0:34
colnames(pointsScored) <- 0:34
...
for(i in 1:length(gameData$game_id)){
  ...
  tryCatch({
    ...
    pointsScored[losing_team_scored,winning_team_scored] <- pointsScored[losing_team_scored,winning_team_scored] + 1
  }, error=function(e){})
...
View(as.table(pointsScored))
pointsScored2 <- pointsScored
pointsScored2[pointsScored2==0] <- NA

plot(pointsScored2, breaks=c(0,1,2,3,5,9,14,50,100,150,1000),col = magma)

x <- vector(mode="numeric", length = 722)
for(i in 0:722) {
  x[i] <- sum(pointsScored>i)
}
sum(pointsScored>14)
sum(pointsScored<=14)
plot(x)
